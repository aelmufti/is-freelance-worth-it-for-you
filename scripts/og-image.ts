...
import { PAGES, ogImagePath, type StatutPage } from "../src/lib/pages";
import { DEFAULT_INPUT, DEFAULT_PARAMS } from "../src/lib/params";
import { calcCdi, calcMicro, calcSasu } from "../src/lib/engine";
...
const HOME_SUB =
  "Comparez micro, EURL, SASU, portage et CDI en net après cotisations et impôt. Trouvez le TJM à partir duquel chaque statut bat votre CDI.";

function htmlFor(page: StatutPage): string {
  const isHome = !page.slug;
  const title = isHome ? HOME_TITLE : esc(page.h1);
  const sub = isHome ? HOME_SUB : esc(page.metaDescription);
  // Taille adaptée à la longueur (les H1 de pages sont plus longs que la home)
  const plain = title.replace(/<[^>]+>/g, "");
  const titleSize = plain.length > 70 ? 56 : plain.length > 48 ? 66 : 86;
  return `<!doctype html>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: "JetBrains Mono", ui-monospace, Menlo, monospace; }
  ...
  h1 { font-size: ${titleSize}px; font-weight: 900; line-height: 1.05; text-transform: uppercase; letter-spacing: -0.02em; color: #0d0d0d; }
